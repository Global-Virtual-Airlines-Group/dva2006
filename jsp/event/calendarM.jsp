...
<a href="javascript:golgotha.local.expandSections('eSignup-${event.hexID}')" class="small ter bld"><fmt:int value="${eventSize}" /> Participants</a>
</c:if>
<c:if test="${!eventLargeSignup}">
<span class="small ter bld"><fmt:int value="${eventSize}" /> Participant<c:if test="${eventSize > 1}">s</c:if></span>
</c:if>
<br>
<div class="eSignup-${event.hexID} small"<c:if test="${eventLargeSignup}"> style="display:none;"</c:if>>
