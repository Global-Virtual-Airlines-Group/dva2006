...
SUM(IF((ATTR & 0x1000000) > 0, 1, 0)) AS SB, SUM(DISTANCE) AS MILES, SUM(FLIGHT_TIME) AS HOURS, COUNT(DISTINCT PILOT_ID) AS PIDS, AVG(LOADFACTOR), SUM(PAX), 
...
SUM(IF((ATTR & 0x1000000) > 0, 1, 0)) AS SB, SUM(DISTANCE) AS MILES, SUM(FLIGHT_TIME) AS HOURS, COUNT(DISTINCT PILOT_ID) AS PIDS, AVG(LOADFACTOR), SUM(PAX) 
FROM PIREPS WHERE (STATUS=3) GROUP BY DATE, NET);

REPLACE INTO FLIGHTSTATS_LANDING (SELECT PR.ID, PR.PILOT_ID, PR.EQTYPE, DATE(APR.LANDING_TIME), PR.AIRPORT_A, APR.LANDING_VSPEED, CAST(R.DISTANCE AS SIGNED) FROM PIREPS PR, 
