...
REPLACE INTO FLIGHTSTATS_LANDSCORE (SELECT R.ICAO, R.RUNWAY, YEAR(P.DATE) AS YR, ROUND(AVG(AP.LANDING_SCORE)), ROUND(STDDEV(AP.LANDING_SCORE)), ROUND(AVG(R.DISTANCE)), ROUND(STDDEV(R.DISTANCE)), 
ROUND(AVG(AP.LANDING_VSPEED)), ROUND(STDDEV(AP.LANDING_VSPEED)), COUNT(R.ID) FROM acars.RWYDATA R LEFT JOIN ACARS_PIREPS AP ON (R.ID=AP.ACARS_ID), PIREPS P WHERE (R.ISTAKEOFF=0) AND (AP.ID=P.ID) AND (AP.LANDING_SCORE>500)
GROUP BY R.ICAO, R.RUNWAY, YR);

REPLACE INTO FLIGHTSTATS_LANDSCORE (SELECT R.ICAO, R.RUNWAY, 0, ROUND(AVG(AP.LANDING_SCORE)), ROUND(STDDEV(AP.LANDING_SCORE)), ROUND(AVG(R.DISTANCE)), ROUND(STDDEV(R.DISTANCE)), ROUND(AVG(AP.LANDING_VSPEED)), 
ROUND(STDDEV(AP.LANDING_VSPEED)), COUNT(R.ID) FROM acars.RWYDATA R LEFT JOIN ACARS_PIREPS AP ON (R.ID=AP.ACARS_ID), PIREPS P WHERE (R.ISTAKEOFF=0) AND (AP.ID=P.ID) AND (AP.LANDING_SCORE>500) GROUP BY R.ICAO, R.RUNWAY);
