...
public class SetFlightReport extends DAO {
   ...
   /**
    * Marks a Flight Report as submitted. The SQL statement is written in such a way that the Flight Report must already
    * be in DRAFT status in order to be submitted, to prevent double-submissions.
   ...
   private void writePromoEQ(int id, String dbName, Collection eqTypes) throws SQLException {
      ...
      prepareStatementWithoutLimits("INSERT INTO " + dbName.toLowerCase() + ".PROMO_EQ (ID, EQTYPE) " + "VALUES (?, ?)");
      _ps.setInt(1, id);
