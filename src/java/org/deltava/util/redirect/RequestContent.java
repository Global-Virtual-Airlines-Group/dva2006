...
/**
 * A bean to store Servlet Request state when redirecting a command. This allows Web Site Commands
 * to save data in the servlet request for display on a result JSP, while at the same time using a redirect
 * so that refreshing the result URL does not invoke the command a second time.
 * @author Luke
 * @version 1.0
 * @since 1.0
...
class RequestContent implements java.io.Serializable {
   ...
   /**
    * Returns the URL to redirect to. The {@link org.deltava.commands.RedirectCommand} will
    * forward to this URL once it has restored the servlet request attributes.
