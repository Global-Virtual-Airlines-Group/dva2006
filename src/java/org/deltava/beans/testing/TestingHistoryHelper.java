...
public class TestingHistoryHelper {
   ...
   /**
    * Returns the Pilot's equipment program. This method is useful when we use this class and do not
    * wish to call the {@link org.deltava.dao.GetEquipmentType} DAO a second time. 
   ...
   /**
    * Returns the Pilot's examinations. Returns the Pilot's equipment program. This method is useful when
   ...
   public int getMaxExamStage() {
      ...
      for (Iterator i = _tests.iterator(); i.hasNext();) {
         Test t = (Test) i.next();
         if (t instanceof Examination) maxStage = Math.max(maxStage, t.getStage());
      }
   ...
   public boolean canWrite(ExamProfile ep) {
      ...
      if (!ep.getActive()) return false;

      // Check if we've passed or submitted the exam
      if (hasPassed(ep.getName()) || hasSubmitted(ep.getName())) return false;

      // Check if we are in the proper equipment program
      if (ep.getEquipmentType() != null)
      	if (!ep.getEquipmentType().equals(_usr.getEquipmentType())) return false;

      // Check if we've reached the proper minimum stage
      if (ep.getMinStage() > getMaxExamStage()) return false;
   ...
   public boolean canSwitchTo(EquipmentType eq) {
      ...
      if ((eq.getStage() > _myEQ.getStage()) && (!_usr.getRank().equals(Ranks.RANK_C))) return false;
   ...
   public boolean canPromote(EquipmentType eq) {
      ...
      if (!_usr.getRank().equals(Ranks.RANK_FO)) return false;

      // Check if we've passed the examination
      if (!hasPassed(eq.getExamName(Ranks.RANK_C))) return false;
   ...
   public boolean hasPassed(String examName) {
      for (Iterator i = _tests.iterator(); i.hasNext();) {
         Test t = (Test) i.next();
         if (t.getPassFail() && (t.getName().equals(examName))) return true;
      }
   ...
   public boolean hasSubmitted(String examName) {
      for (Iterator i = _tests.iterator(); i.hasNext();) {
         Test t = (Test) i.next();
         if ((t.getStatus() == Test.SUBMITTED) && (t.getName().equals(examName))) return true;
      }
