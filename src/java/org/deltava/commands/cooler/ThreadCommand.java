...
public class ThreadCommand extends AbstractCommand {
    ...
    public void execute(CommandContext ctx) throws CommandException {
        ...
        try {
            ...
            ac.validate();
            if (!ac.getCanRead())
                throw new CommandSecurityException("Cannot read Message Thread " + ctx.getID());
            
            // If we have an image, load its metadata
            if (thread.getImage() != 0) {
            	GetGallery irdao = new GetGallery(con);
            	Image img = irdao.getImageData(thread.getImage());
            ...
            if (ac.getCanUnlock() || ac.getCanLock()) {
                ...
                ctx.setAttribute("channels", cdao.getChannels(airline), REQUEST);
            }
