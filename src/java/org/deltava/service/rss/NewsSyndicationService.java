// Copyright 2005, 2006, 2008, 2012, 2015, 2021, 2023, 2025 Global Virtual Airlines Group. All Rights Reserved.
...
public class NewsSyndicationService extends SyndicationService {
	...
	public int execute(ServiceContext ctx) throws ServiceException {
		...
		Document doc = initRSS(String.format("%s News", alName), String.format("What's New at %s", alName), String.format("https://%s/news.do", serverName));
		...
		for (News n : entries) {
			try {
				URI url = new URI("https", serverName, "/news.do?id=" + StringUtils.formatHex(n.getID()));
