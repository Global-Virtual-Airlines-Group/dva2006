...
public class NavigationAidService extends DispatchDataService {
	...
	public int execute(ServiceContext ctx) throws ServiceException {
		...
		Navaid type = Navaid.values()[StringUtils.parse(ctx.getParameter("type"), Navaid.VOR.ordinal())];
		File f = _dataCache.get(type);
		if (f != null) {
		...
		f = new File(cacheDir, "navaid" + type.getName() + ".txt");
		...
		try {
			try (PrintWriter pw = new PrintWriter(new BufferedOutputStream(new FileOutputStream(f), 65536))) {
				...
				for (Iterator<NavigationDataBean> i = navaids.iterator(); i.hasNext();) {
					...
					pw.print(nb.getCode() + "." + nb.getType().getName() + "=");
					pw.print(df.format(nb.getLatitude()) + "," + df.format(nb.getLongitude()));
					if ((nb.getType() == Navaid.VOR) || (nb.getType() == Navaid.NDB)) {
						pw.print("," + nb.getName() + ",");
