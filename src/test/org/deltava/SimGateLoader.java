...
public class SimGateLoader extends SceneryLoaderTestCase {
	...
	public void testLoadXML() throws Exception {
		...
		PreparedStatement ps = _c.prepareStatement("REPLACE INTO common.GATES (ICAO, NAME, SIMVERSION, LATITUDE, LONGITUDE, HDG) VALUES (?, ?, ?, ?, ?, ?)");
		...
		for (int x = 0; x < xmls.length; x++) {
			...
			while (ali.hasNext()) {
				...
				if (!codes.contains(apCode)) continue;
